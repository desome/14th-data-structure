...
public:
	...
	int numPathsDAG() {
		...
		tmp= helpnumPathsDAG(tmp, numpath);
		delete[] numpath;
		return tmp;
	}

	int helpnumPathsDAG(int point, int *numpath) {
		if (numpath[point] > 0)
			return numpath[point];

		int sum = 0;
		for (int j = 0; j < numVer; j++) {
			if (adjMatrix[j][point] > 0)
			{
				numpath[j] = helpnumPathsDAG(j, numpath);
				sum += numpath[j];
			}
		}
